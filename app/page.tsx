...
function AnnouncementBar() {
  return (
    <div className="border-b border-white/[0.06] bg-white/[0.015]">
      <Container className="flex items-center justify-center gap-2 py-2 text-center text-[12px] text-white/55">
        <span className="h-1.5 w-1.5 rounded-full bg-faceit-orange" />
        Trusted by <span className="font-semibold text-white">{site.stats.users}</span> FACEIT
        players catching cheaters
      </Container>
    </div>
  );
}
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-white/[0.06] bg-ink/70 backdrop-blur-xl">
      <Container className="flex items-center justify-between py-3.5">
        ...
        <CtaButton className="!px-4 !py-2.5 text-[13px]">
...
function Hero() {
  return (
    <section className="relative">
      <Container className="grid items-center gap-16 pb-20 pt-16 md:grid-cols-[1.05fr_0.95fr] md:pb-28 md:pt-24">
        <div className="rise">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.02] px-3 py-1.5">
            <span className="h-1.5 w-1.5 rounded-full bg-faceit-orange" />
            <span className="text-xs font-medium text-white/70">For FACEIT · CS2</span>
          </div>

          <h1 className="font-display text-[2.75rem] font-bold leading-[0.98] tracking-[-0.02em] sm:text-6xl">
            They got banned.
            <br />
            <span className="text-faceit-orange">Find the match instantly.</span>
          </h1>
          ...
          <p className="mt-6 max-w-md text-[17px] leading-relaxed text-white/60">
            ...
            exact match you shared with them — open it, review it, report it. One
            click, no scrolling through months of history.
          </p>
          ...
          <div className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <a
              href="#how"
              className="inline-flex items-center gap-1.5 px-2 text-sm font-medium text-white/70 transition-colors hover:text-white"
            >
              See how it works
    ...
    <section className="border-y border-white/[0.06] bg-white/[0.012]">
      <Container>
        <dl className="grid grid-cols-2 divide-white/[0.06] md:grid-cols-4 md:divide-x">
          {STATS.map((s, i) => (
            <div key={s.label} className={`py-8 ${i % 2 === 0 ? "pr-4" : "pl-4 md:pl-8"} md:px-8`}>
              <dt className="font-display text-3xl font-bold tracking-tight text-white sm:text-4xl">
                {s.value}
              </dt>
              <dd className="mt-1.5 text-sm text-white/50">{s.label}</dd>
            </div>
          ))}
        </dl>
      </Container>
  ...
  return (
    <section className="py-24 sm:py-28">
      <Container>
        ...
        <p className="mt-6 max-w-4xl font-display text-[1.6rem] font-medium leading-[1.25] tracking-[-0.01em] text-white/80 sm:text-[2.15rem]">
          <span className="text-white/35">You reported them. You were right.</span> But FACEIT
          confirmed it days later — and by then the ELO was gone and the match was buried under a
          hundred others. <span className="text-faceit-orange">CheckMate finds it in one click.</span>
        </p>
      </Container>
    </section>
  );
...
function HowItWorks() {
  return (
    <section id="how" className="border-t border-white/[0.06] bg-white/[0.012] py-24 sm:py-28">
      <Container>
        <div className="max-w-2xl">
          <Eyebrow>How it works</Eyebrow>
          <h2 className="mt-4 font-display text-3xl font-bold tracking-[-0.02em] sm:text-[2.5rem]">
          ...
          {STEPS.map((s) => (
            <div key={s.n} className="relative">
              <div className="flex items-center gap-4">
                <span className="font-display text-2xl font-bold text-faceit-orange">{s.n}</span>
      ...
      <Container>
        ...
        <div className="mt-14 grid gap-4 md:grid-cols-3">
          {BENEFITS.map((b) => (
            <div
              key={b.title}
              className="group rounded-2xl border border-white/[0.08] bg-white/[0.015] p-7 transition-colors hover:border-white/15 hover:bg-white/[0.03]"
            >
              <div className="grid h-11 w-11 place-items-center rounded-xl bg-faceit-orange/10 text-faceit-orange ring-1 ring-faceit-orange/20">
...
function FounderNote() {
  return (
    <section className="border-y border-white/[0.06] bg-white/[0.012] py-24 sm:py-28">
      <Container>
        <div className="mx-auto max-w-3xl">
          <Eyebrow>Why it exists</Eyebrow>
          <blockquote className="mt-6 font-display text-2xl font-medium leading-[1.35] tracking-[-0.01em] text-white/85 sm:text-[1.7rem]">
            &ldquo;I kept getting the &lsquo;player banned, thanks for your report&rsquo; notification
            days after the match — and I could never find which game it was to get my ELO reviewed.
            So I built CheckMate to do it in one click. It&apos;s free because it started as a fix for
            my own problem.&rdquo;
          </blockquote>
          <div className="mt-7 flex items-center gap-3.5">
            <BrandLogo size={40} />
            <div>
              <p className="text-sm font-semibold text-white">{site.author}</p>
              <p className="text-sm text-white/50">Creator of FACEIT CheckMate</p>
            </div>
          </div>
        </div>
      </Container>
  ...
  return (
    <section id="faq" className="py-24 sm:py-28">
      <Container className="grid gap-12 md:grid-cols-[0.8fr_1.2fr]">
        ...
        <div>
          {FAQS.map((f) => (
            <details key={f.q} className="group border-b border-white/[0.08] py-5">
              <summary className="flex cursor-pointer list-none items-center justify-between gap-4 font-display text-[17px] font-semibold text-white/90 transition-colors hover:text-white">
                {f.q}
...
function Feedback() {
  return (
    <section id="feedback" className="border-t border-white/[0.06] bg-white/[0.012] py-24 sm:py-28">
          ...
          <ul className="mt-6 space-y-2.5 text-sm text-white/55">
            {[
              ...
              <li key={t} className="flex items-start gap-2.5">
                <span className="mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full bg-faceit-orange" />
                {t}
              </li>
            ))}
          </ul>
...
function FinalCta() {
  return (
    <section className="pb-24">
      <Container>
        <div className="relative overflow-hidden rounded-3xl border border-white/[0.08] bg-gradient-to-b from-white/[0.04] to-white/[0.01] px-6 py-16 text-center sm:py-20">
          <div className="pointer-events-none absolute inset-x-0 top-0 h-40 bg-[radial-gradient(60%_100%_at_50%_0%,rgba(255,85,0,0.18),transparent_70%)]" />
          <div className="relative">
            <h2 className="mx-auto max-w-2xl font-display text-4xl font-bold leading-[1] tracking-[-0.02em] sm:text-[3.25rem]">
...
function Footer() {
  ...
  return (
    <footer className="border-t border-white/[0.06] bg-white/[0.012]">
      <Container className="py-14">
        ...
        <div className="mt-12 flex flex-col gap-3 border-t border-white/[0.06] pt-7 text-[13px] text-white/40 sm:flex-row sm:items-center sm:justify-between">
          <p>
...
function FooterCol({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div>
      <p className="font-mono text-[11px] font-medium uppercase tracking-[0.2em] text-white/35">
        {title}
      </p>
